...
把一个主题、一句话、一个物件或一张照片，转成原创的编辑印刷作品。Skill 默认使用受控双色：一块主墨版承担主体，另一块辅墨版只负责一个明确事件。用户明确要求单色、单墨或只指定一种油墨时，仍使用纯单色。暖色纸张、网点图像、主动留白与强烈字体对比，让两种模式保持同一套视觉识别。
